...
namespace HW8_2
{
    class BaseOfGame
    {
        ...
        public bool QuestionAnswerConsole(int indexQuestion)
        {
        ...
        /// </summary>
        /// <param name="indexQuestion">Номер вопроса (от 0)</param>
        /// <returns>Возвращает pair (string и bool) из вопроса и ответа.</returns>
        public KeyValuePair<string, bool> QuestionAnswer(int indexQuestion)
        {
            KeyValuePair<string, bool> QA1 = new KeyValuePair<string, bool>(string.Empty, false);

            if (dataBase.GetLength(0) > indexQuestion && indexQuestion >= 0)
            {
                string tr = "Н";
                if (tr.ToLower()[0] == dataBase[indexQuestion, 1].ToLower()[0])
                    QA1 = new KeyValuePair<string, bool>(dataBase[indexQuestion, 0], false);
                else
                    QA1 = new KeyValuePair<string, bool>(dataBase[indexQuestion, 0], true);
            }
            return QA1;
        }
    }
